'use strict';
(function() {
  const gl = makeGL('cvsTransformation');
  if (!gl) return;
  ...
  function draw() {
    ...
    const model = m4.multiply(m4.translate(state.x, state.y, 0),
  ...
  draw();
  document.getElementById('cvsTransformation').addEventListener('keydown', e => {
    switch(e.key) {
